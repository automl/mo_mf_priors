...
#SBATCH --job-name MO_naive_priors
#SBATCH --output logs/%x-%A_%a_meta.out
#SBATCH --error logs/%x-%A_%a_meta.err
#SBATCH --cpus-per-task 30
#SBATCH --array=0-35%20   # (1 opt * 4 priors) * 9 benchmarks = 36 total combinations
...
python3 -m momfpriors.run -y "$yaml_file" -e "MO_priors_naive_20"
