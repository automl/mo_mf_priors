...
#SBATCH --job-name ablation_hwgpt
#SBATCH --output logs/%x-%A_%a.out
#SBATCH --error logs/%x-%A_%a.err
#SBATCH --cpus-per-task 30
#SBATCH --array=0-38%18   # (3 prior opts * 4 priors + 1 non-prior opts) * 3 benchmarks = 39 total combinations
#SBATCH --time=4-00:00:00

echo "Workingdir: $PWD"
echo "Started at $(date)"

source ~/repos/hwgpt_env/bin/activate
...
# === Run the experiment ===
python3 -m momfpriors.run -y "$yaml_file" -e "ablation1_20" --data_dir "$data_dir" --priors_dir "$priors_dir"
