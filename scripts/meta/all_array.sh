...
#SBATCH --job-name mfzdt_25
#SBATCH --output logs/%x-%A_%a_meta.out
#SBATCH --error logs/%x-%A_%a_meta.err
#SBATCH --cpus-per-task 30
#SBATCH --array=0-36%20   # (4 prior opts * 3 priors + 6 non-prior opts) * 2 benchmarks = 36 total combinations
...
mkdir -p "$config_dir"
yaml_file="${config_dir}/mfzdt_${SLURM_ARRAY_TASK_ID}.yaml"
...
# === Run the experiment ===
python3 -m momfpriors.run -y "$yaml_file" -e "mfzdt_all_25"
